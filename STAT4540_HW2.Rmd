...
Therefore, $\boxed{\textrm{MSE}_{\textrm{min}} = \gamma(0)[1-\rho(\ell)^2]}$

#### Part C

To show that if $x_{t+\ell} =Ax_t$, $\rho(\ell) =1\ \textrm{if}\ A > 0$ and $\rho(\ell) =-1,\ \textrm{if}\ A<0$ we first define $\rho(\ell)$ as:

$$\rho(\ell)=\frac{\gamma(\ell)}{\gamma(0)} = \frac{\textrm{Cov}(x_{t+\ell},x_t)}{\textrm{Var}(x_t)}$$

Where we can sub $x_{t+\ell} =Ax_t$ in and get:

$$\rho(\ell) = \frac{\textrm{Cov}(Ax_t,x_t)}{\textrm{Var}(x_t)} =\frac{A\textrm{Var}(x_t)}{\textrm{Var}(x_t)} = A$$

From Part B, we know that $\textrm{MSE}_{\textrm{min}}=\gamma(0)[1-\rho(\ell)^2]=0$ and solving for $\rho(\ell) = \pm1$. This means that because $\rho(\ell) = A$ then when $A$ has a positive sign, that $\rho(\ell) =1$ likewise when $A<0,\ \rho(\ell)=-1$.
